...
`main()`的第一行创建了一个`ArrayList`对象，然后将其向上转换成为一个集合。由于这个例子只使用了`Collection`方法，所以从`Collection`继承的一个类的任何对象都可以正常工作。但`ArrayList`是一个典型的`Collection`，它代替了`Vector`的位置。 显然，`add()`方法的作用是将一个新元素置入集合里。然而，用户文档谨慎地指出`add()`“保证这个集合包含了指定的元素”。这一点是为`Set`作铺垫的，后者只有在元素不存在的前提下才会真的加入那个元素。对于`ArrayList`以及其他任何形式的`List`，`add()`肯定意味着“直接加入”。
